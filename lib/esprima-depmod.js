...
var cssModuleName = /html\/(?:.*)\/(.+)\.css/;
var libCssModuleName = /html\/lib\/(.+)\.css/;
...
function processCSS(filename) {
    ...
    // TODO: remove this after CSS refactor, leave just "else" branch
    if (filename.search('/lib/') != -1) {
        name += libCssModuleName.exec(filename)[1];
        name = name.replace(/\//g, '_');
    } else {
        name += cssModuleName.exec(filename)[1];
    }
...
function depmodEntry(name, filename, requires) {
    ...
    // TODO: m.path is now fucked up due to wierd paths in our former depmod - refactor
    // path is generated in a way that it is relative to tools/resolver/lib/resolver/cli
    // WTF? Yeah, exactly!
    var m = {
        name: name,
        path: require('path').relative('tools/resolver/lib/resolver/', filename).replace('/html', ''),
        ext: false,
